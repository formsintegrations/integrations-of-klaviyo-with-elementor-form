...
import {
  refreshAssigmentRules,
  refreshOwners,
  refreshTags,
} from "./ZohoCRMCommonFunc";
...
export default function ZohoCRMActions({
  ...
  const isPro = typeof iklaviyoef !== "undefined" && iklaviyoef.isPro;
  ...
  const actionHandler = (val, typ) => {
    ...
    if (tab === 0) {
      ...
      if (typ === "upsert") {
        if (val.target.checked) {
          const crmField = newConf.default.layouts[newConf.module][
            newConf.layout
      ...
      if (typ === "upsert") {
        if (val.target.checked) {
          const crmField = newConf.default.layouts[
            newConf.relatedlists[tab - 1].module
          ][newConf.relatedlists[tab - 1].layout].unique?.map((name, i) => ({
            i,
            name,
          }));
          newConf.relatedlists[tab - 1].actions.upsert = {
            overwrite: true,
            crmField,
          };
        } else {
  ...
  const module =
    tab === 0 ? crmConf.module : crmConf.relatedlists[tab - 1].module;
  const getTags = () => {
    const arr = [
    ...
    if (crmConf.default.tags?.[module]) {
      arr[0].childs = Object.values(crmConf.default.tags?.[module]).map(
        (tagName) => ({ label: tagName, value: tagName })
      );
    }

    arr[1].childs = formFields.map((itm) => ({
      label: itm.label,
      value: `\${${itm.name}}`,
    }));
    return arr;
    ...
    <div className="pos-rel">
      ...
      <div className="d-flx flx-wrp">
        ...
        <TableCheckBox
          ...
          subTitle={__(
            "Add attachments or signatures from BitForm to CRM.",
            "bit-integrations"
          )}
        />
        <TableCheckBox
          ...
          subTitle={__(
            "Send entries to CRM approval list.",
            "bit-integrations"
          )}
        />
        <TableCheckBox
        ...
        <TableCheckBox
          ...
          subTitle={__(
            "Sends the click details of AdWords Ads to Zoho CRM.",
            "bit-integrations"
          )}
        />
        ...
        <TableCheckBox
          ...
          subTitle={__(
            "Trigger Assignment Rules in Zoho CRM.",
            "bit-integrations"
          )}
        />
        <TableCheckBox
          ...
          subTitle={__(
            "Add a tag to records pushed to Zoho CRM.",
            "bit-integrations"
          )}
        />
        <TableCheckBox
          ...
          subTitle={__(
            "Add a owner to records pushed to Zoho CRM.",
            "bit-integrations"
          )}
        />
      </div>
        ...
        {isLoading ? (
          ...
          <div className="flx flx-between mt-2">
            <select
              ...
              onChange={(e) =>
                actionHandler(e.target.value, "assignment_rules")
              }
            >
              <option value="">
                {__("Select Assignment Rule", "bit-integrations")}
              </option>
              {crmConf?.default?.assignmentRules?.[module] &&
                Object.keys(crmConf.default.assignmentRules[module]).map(
                  (assignmentName) => (
                    <option
                      key={assignmentName}
                      value={
                        crmConf.default.assignmentRules[module][assignmentName]
                      }
                    >
                      {assignmentName}
                    </option>
      ...
      >
        ...
        <small>
          {__("Add a tag to records pushed to Zoho CRM", "bit-integrations")}
        </small>
            ...
            <MultiSelect
              ...
              defaultValue={
                tab === 0
                  ...
                  : crmConf.relatedlists[tab - 1].actions.tag_rec
              }
              options={getTags()}
        ...
        <div className="o-a">
          {tab === 0
            ? crmConf?.actions?.upsert && (
                <>
                  <div className="font-w-m mt-2">
                    {__("Upsert Using", "bit-integrations")}
                  </div>
                  <small>
                    {__(
                      "Arrange fields in order of preferance for upsertion",
                      "bit-integrations"
                    )}
                  </small>
                  <ReactSortable
                  ...
                  >
                    {crmConf.actions.upsert?.crmField?.map((itm) => (
                      <div
                        key={`cf-${itm.i}`}
                        className="upsert_rec w-7 mt-1 flx"
                      >
                  ...
                  <div className="font-w-m mt-3">
                    {__("Upsert Preferance", "bit-integrations")}
                  </div>
                  <small>
                    {__(
            ...
            : crmConf?.relatedlists[tab - 1]?.actions?.upsert && (
                <>
                  ...
                  <ReactSortable
                    list={
                      crmConf.relatedlists[tab - 1].actions.upsert?.crmField
                    }
                    setList={(l) => setUpsertSettings(l, "list")}
                  >
                    {crmConf.relatedlists[
                      tab - 1
                    ].actions.upsert?.crmField?.map((itm) => (
                      <div
                        key={`cf-${itm.i}`}
                        className="upsert_rec w-7 mt-1 flx"
                      >
                  ...
                  <div>
                    <CheckBox
                      ...
                      radio
                      checked={
                        crmConf.relatedlists[tab - 1].actions.upsert?.overwrite
                      }
                      name="up-rec"
                      title={__("Yes", "bit-integrations")}
                    />
