...
const AllFormContextProvider = (props) => {
  ...
  if (typeof iklaviyoef !== "undefined" && iklaviyoef.allForms !== null) {
    allFormsInitialState = iklaviyoef?.allForms?.map((form) => ({
      formID: form.id,
      status: form.gclid,
      formName: form.title,
  ...
  const [allForms, allFormsDispatchHandler] = useReducer(
    AllFormsDispatchHandler,
    allFormsInitialState
  );
