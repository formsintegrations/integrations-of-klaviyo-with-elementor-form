...
function AllIntegrations() {
  ...
  useEffect(() => {
    ...
    ncols.push({ sticky: 'right', width: 100, minWidth: 60, Header: 'Actions', accessor: 't_action', Cell: val => <MenuBtn id={val.row.original.id} name={val.row.original.name} index={val.row.id} del={() => showDelModal(val.row.original.id, val.row.index)} /> })
    setCols([...ncols])
  }, [integrations])
    ...
    <div id="all-forms">
      ...
      >
        ...
        <div className="txt-center">
          <a href="https://www.bitapps.pro/bit-integrations" target="_blank" rel="noreferrer"><button className="btn btn-lg blue" type="button">{__('Buy Premium')}</button></a>
        </div>
